...
│  │  Query: "数字万用表 认证标准"                                 │ │
│  │                                                              │ │
│  │  候选1: "某电子公司全线产品通过欧盟EMC(电磁兼容)、LVD(低电压..." │ │
│  │    → 相关性: 0.92 ✓ (直接匹配认证信息)                        │ │
│  │                                                              │ │
...
```
┌──────────────────────────────────────────────────────────────────────────────────────────┐
│                          用户消息: "帮 Example Tech GmbH 生成询盘邮件"                        │
└──────────────────────────────────────────────────────────────────────────────────────────┘
                                          │
                                          ▼
┌──────────────────────────────────────────────────────────────────────────────────────────┐
...
│  │                                                                                        │ │
│  │  → reasoning_content: "用户要给 Example Tech GmbH 生成邮件，我需要先搜索客户..."          │ │
│  │    yield SSE: event: thinking                                                          │ │
│  │                                                                                        │ │
│  │  → tool_calls: [search_customers(query="example tech")]                            │ │
...
│  │   → SQLite: SELECT * FROM customer WHERE company_name LIKE '%example tech%'       │ │
│  │   → 返回 5 条匹配: [id=11 purchase@example.com, id=73 info@example.com, ...]              │ │
...
│  ┌──────────────────────────────────────────────────────────────────────────────────────┐ │
│  │ 返回确认卡片                                                                            │ │
│  │   yield SSE: event: content {text: "已为 Example Tech GmbH... 生成邮件草稿..."}          │ │
